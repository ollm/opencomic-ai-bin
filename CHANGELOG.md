# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## v1.4.6

##### 🚀 New Features

- OpenComic AI V2 models

## v1.4.5

##### 🚀 New Features

- Add support for registering custom models with `OpenComicAI.setCustomModel()`.

##### 🐛 Bug Fixes

- Fix pipelines that combine `keepBigHalftone` with a following upscale step by passing the corrected intermediate image to the next stage.

## v1.4.0

##### 🚀 New Features

- Preliminary support for Panel detection and Descreen Mask

## v1.3.0

##### 🚀 New Features

- Preliminary support for YOLO26 models [`0401cb7`](https://github.com/ollm/opencomic-ai-bin/commit/0401cb79668705d32ddfa08eee6f2786121a1ed5)
- Add OpenComic AI Upscale Lite model [`6641ea6`](https://github.com/ollm/opencomic-ai-bin/commit/6641ea6f65f2dd0a0a517eb388bffb3bbd9a431a)

## v1.2.0

##### 🚀 New Features

- Add some of the OpenComic AI models [`ee94534`](https://github.com/ollm/opencomic-ai-bin/commit/ee945348b83f4f1420b35c114fefd763ed13237d)

## v1.1.4

##### 🐛 Bug Fixes

- Windows binary for `upscayl-bin` (daemon mode not working) [`b348e37`](https://github.com/ollm/opencomic-ai-bin/commit/b348e3754245cb633d02937d35a7270e53670c3a)

## v1.1.3

##### 🚀 New Features

- Improved cache of daemon mode and updated `upscayl-bin` binaries [`75068f3`](https://github.com/ollm/opencomic-ai-bin/commit/75068f379366c841acfbdb3509be838a8afb7843)

## v1.1.2

##### 🚀 New Features

- Add Windows arm64 binary for `upscayl-bin` [`16b3949`](https://github.com/ollm/opencomic-ai-bin/commit/16b3949b2890b42003079f1b63d27cf16b449497)

##### 🐛 Bug Fixes

- Escape quotes in source and destination paths for process input [`8b14690`](https://github.com/ollm/opencomic-ai-bin/commit/8b14690dcd77dcea1a8d37dfe51b9487294df0da)

## v1.1.1

##### 🚀 New Features

- Add Linux arm64 binary for `upscayl-bin` [`52889e5`](https://github.com/ollm/opencomic-ai-bin/commit/52889e56c7e8cfae07c159600fee989244bc7033)

## v1.1.0

##### 🚀 New Features

- Daemons to speed up processing loading model only once and not for each image (`upscayl` models only) [`69660d7`](https://github.com/ollm/opencomic-ai-bin/commit/69660d7136aaa6e01b38da650fc0281f786aa278)